...
> 📐 **[열 개로 한 걸음씩 — 인터랙티브 데모](block_sort.html)**
> 무작위 숫자 10개가 정렬되는 과정을 한 단계씩 따라갈 수 있다. 회전이 왜 뒤집기
> 세 번인지, 제자리 병합이 어떻게 문제를 쪼개는지는 글보다 그쪽이 빠르다.
...
| --- | --- | ---: | ---: | ---: | ---: |
| 무작위 | insertionSort | 11.026 | 3,950,647 | 3,950,655 | 1 |
| 무작위 | bubbleSort | 29.563 | 7,997,922 | 11,828,007 | 1 |
| 무작위 | **blockSort** | **0.697** | **115,971** | 352,201 | 16 |
| 정렬됨 | insertionSort | 0.004 | 3,999 | 0 | 1 |
| 정렬됨 | bubbleSort | 0.005 | 3,999 | 0 | 1 |
| 정렬됨 | blockSort | 0.005 | 3,999 | 0 | 1 |
| 역순 | insertionSort | 20.816 | 8,001,999 | 8,005,998 | 1 |
| 역순 | bubbleSort | 30.629 | 7,998,000 | 23,994,000 | 1 |
| 역순 | **blockSort** | **0.530** | **133,851** | 311,166 | 24 |
| 중복많음 | insertionSort | 9.037 | 3,575,497 | 3,575,032 | 1 |
| 중복많음 | bubbleSort | 27.812 | 7,867,695 | 10,703,898 | 1 |
| 중복많음 | **blockSort** | **0.449** | **74,499** | 238,678 | 21 |

##### 걸린 시간

![입력 모양에 따른 걸린 시간 — 선형 축](../docs/input-shapes-time.svg)

- **이것이 실제 격차다.** 버블 정렬만 우뚝하고, 블록 정렬은 바닥의 선으로만 남는다.
- `정렬됨` 묶음에는 막대가 보이지 않는다. 셋 다 0.004ms 언저리라 높이가 없다.
  조기 종료가 셋 다 걸리기 때문이다. **정렬의 성능은 입력을 빼고 말할 수 없다.**

![입력 모양에 따른 걸린 시간 — 로그 축](../docs/input-shapes-time-log.svg)

- 같은 자료다. 로그 축에서는 0.004ms도 자기 높이를 갖는다.
- 블록 정렬이 `정렬됨`을 빼면 세 묶음에서 거의 같은 높이(0.449~0.697ms)라는 것도
...
![입력 모양에 따른 이동 횟수 — 선형 축](../docs/input-shapes-moves.svg)

- 버블 정렬이 역순에서 `23,994,000`회로 혼자 치솟는다.
- `정렬됨`은 0회다. 셋 다 한 원소도 옮기지 않는다.

![입력 모양에 따른 이동 횟수 — 로그 축](../docs/input-shapes-moves-log.svg)
...
![n이 커질 때 비교 횟수 — 선형 축](../docs/growth-compares.svg)

- 삽입·버블은 위로 휘어 오르고, 블록 정렬은 바닥에 붙어 거의 평평하다.
- `n`이 두 배가 될 때마다 간격이 네 배로 벌어진다. 표의 `×3.96`, `×4.04`가 이 휘어짐이다.

![n이 커질 때 비교 횟수 — 로그-로그 축](../docs/growth-compares-log.svg)

- 로그-로그에서는 셋 다 **직선**이 되고, **기울기가 곧 복잡도 지수**가 된다.
- 삽입 `1.99`, 버블 `2.00` — `O(n^2)`의 지수 2를 축에서 바로 읽는다.
- 블록 정렬은 `1.32`다. `n log n`이면 1.1쯤이어야 하니, 조금 가파른 것이 `log^2` 항의 값이다.

| n | insertion(ms) | bubble(ms) | block(ms) |
| ---: | ---: | ---: | ---: |
| 1,000 | 0.669 | 1.782 | 0.143 |
| 2,000 | 2.823 | 7.329 | 0.346 |
| 4,000 | 11.139 | 30.318 | 0.832 |
| 8,000 | 42.680 | 120.978 | 1.791 |

![n이 커질 때 걸린 시간 — 선형 축](../docs/growth-time.svg)

`n = 8,000`에서 블록 정렬은 버블 정렬보다 **68배** 빠르다.
그래프에서 블록 정렬 선이 x축과 구별되지 않는 것이 그 배수다.

![n이 커질 때 걸린 시간 — 로그-로그 축](../docs/growth-time-log.svg)
