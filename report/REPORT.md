...
된다. 왼쪽으로 보내면 바로 앞의 이어붙이기 검사에서 `a[mid-1] > a[mid]`임이 이미
확인됐으므로 `lowerBound`가 반드시 `mid`보다 큰 자리를 돌려주어 회전이 비지 않는다.
...
표의 값은 비교 횟수다(입력이 고정이라 재현된다).

![블록 크기를 바꿔 가며 — 비교 횟수](block-size.svg)

**최적은 `n`을 따라가지 않는다.** `n`이 1,000이든 64,000이든 8~32에서 가장 낮고 거기서
평평하다. 반면 블록을 키우면 비교 횟수가 곧장 치솟는다.
...
`n = 64,000`에서 비교를 **1.1배** 더 쓰고 있었다.

지금은 **32 상수**를 쓴다(`src/blockSort.c`의 `blockSortBlockSize`). 8이 비교는 조금 더
적지만 이동이 많아 시간은 비슷하고, 32는 평평한 구간의 가운데다.
