 ...
 * 준다. 그래서 최적 B는 n과 무관한 작은 상수다. 재 보면 최저는 어느 n에서나 8이고
 * 32까지는 완만하게 오른다 (보고서 3.2 「블록 크기는 얼마가 좋은가」).
 * timsort의 minrun이 32~64인 것도 같은 이유다.
 *
...
static void mergeInPlace(SortCtx *c, size_t lo, size_t mid, size_t hi, size_t depth) {
     ...
     * 비고, 같은 인자로 다시 불려 무한 재귀가 된다. 왼쪽으로 보내면 그때
     * 고르는 자리가 곧 a[mid-1]이고, 앞의 이어붙이기 검사에서
     * a[mid-1] > a[mid] 임이 이미 확인됐으므로 lowerBound가 mid 너머를
     * 돌려준다.
     *
     * 런이 더 길면 회전이 비기도 한다(j == mid). 그래도 고른 자리가 lo보다
     * 뒤라 재귀는 짧아진다 — 진행을 보장하는 것은 아래 assert의 newMid > lo다. */
    if (mid - lo >= hi - mid) {
