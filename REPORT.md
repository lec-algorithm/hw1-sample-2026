...
| `docs/` | 그래프와 측정값 원본 (`make charts`가 다시 만든다) |
...
make charts             # 그 CSV로 docs/ 아래에 그래프를 그린다
```
...
실행**에서 나왔고, 그때의 측정값은 [`docs/results.csv`](docs/results.csv)에 남겼다.
...
| --- | --- | ---: | ---: | ---: | ---: |
| 무작위 | insertionSort | 10.536 | 3,950,647 | 3,950,655 | 1 |
| 무작위 | bubbleSort | 28.188 | 7,997,922 | 11,828,007 | 1 |
| 무작위 | **blockSort** | **0.747** | **115,971** | 352,201 | 16 |
| 정렬됨 | insertionSort | 0.004 | 3,999 | 0 | 1 |
| 정렬됨 | bubbleSort | 0.005 | 3,999 | 0 | 1 |
| 정렬됨 | blockSort | 0.004 | 3,999 | 0 | 1 |
| 역순 | insertionSort | 21.379 | 8,001,999 | 8,005,998 | 1 |
| 역순 | bubbleSort | 30.383 | 7,998,000 | 23,994,000 | 1 |
| 역순 | **blockSort** | **0.617** | **133,851** | 311,166 | 24 |
| 중복많음 | insertionSort | 14.277 | 3,575,497 | 3,575,032 | 1 |
| 중복많음 | bubbleSort | 27.349 | 7,867,695 | 10,703,898 | 1 |
| 중복많음 | **blockSort** | **0.431** | **74,499** | 238,678 | 21 |

표의 네 열을 각각 그래프로 옮겼다. 시간·비교·이동은 세로축이 **로그**다.
정렬된 입력과 최악의 차이가 서너 자릿수라, 선형 축에서는 짧은 막대가 모두 바닥에
붙어 버린다.

#### 걸린 시간

![입력 모양에 따른 걸린 시간](docs/input-shapes-time.svg)

- 무작위 · 역순 · 중복많음 세 묶음에서 **높이의 순서가 늘 같다.** 버블 > 삽입 > 블록.
- `정렬됨` 묶음만 셋이 나란히 주저앉는다(0.005ms 언저리). 조기 종료가 셋 다
  걸리기 때문이다. **정렬 알고리즘의 성능은 입력을 빼고 말할 수 없다.**
- 블록 정렬 막대는 `정렬됨`을 빼면 세 묶음에서 거의 같은 높이다(0.431~0.747ms).
  입력 모양을 타지 않는다는 뜻이고, 이것이 최악을 감당해야 하는 코드가 원하는 성질이다.

#### 비교 횟수

![입력 모양에 따른 비교 횟수](docs/input-shapes-compares.svg)

- **정렬된 입력에서는 셋이 똑같다.** 비교 `n-1`번, 이동 0번.
- **역순은 `O(n^2)` 둘의 최악이다.** 삽입 정렬의 비교가 `8,001,999`로
  이론값 `n^2/2 = 8,000,000`과 맞는다.
- **블록 정렬은 중복이 많으면 오히려 빨라진다.** 조기 종료가 더 자주 걸린다.

#### 이동 횟수

![입력 모양에 따른 이동 횟수](docs/input-shapes-moves.svg)

- `정렬됨`에는 **막대가 아예 없다.** 0회라서다. 셋 다 한 원소도 옮기지 않는다.
- 나머지 세 묶음에서 버블 정렬만 한 칸 위에 있다. 비교 그래프에서는 삽입 정렬과
  거의 같은 높이였는데도 그렇다.

두 그래프를 겹쳐 보면 버블 정렬이 지는 이유가 한 장에 들어온다.

![비교 횟수는 비슷한데 이동 횟수가 다르다](docs/compares-vs-moves.svg)

역순 입력에서 둘의 비교 횟수는 `8,001,999` 대 `7,998,000`로 거의 같다.
...
![입력 모양에 따른 재귀 깊이](docs/input-shapes-depth.svg)

- 삽입·버블은 반복문뿐이라 **어떤 입력에서도 1**이다.
- 블록 정렬만 입력을 탄다. `정렬됨`에서는 1(병합이 즉시 끝난다), 역순에서는 24까지 간다.
- 이 열이 블록 정렬이 치른 값이다. 메모리 열이 셋 다 8 B로 같은 대신, 스택은 여기로 나간다.
...
| 2,000 | 998,714 | ×3.91 | 1,998,945 | ×4.01 | 45,915 | ×2.45 |
| 4,000 | 3,950,647 | ×3.96 | 7,997,922 | ×4.00 | 115,971 | ×2.53 |
| 8,000 | 15,966,683 | ×4.04 | 31,982,470 | ×4.00 | 289,630 | ×2.50 |

![비교 횟수가 n에 따라 자라는 모양](docs/growth-compares.svg)

로그-로그 축이라 **기울기가 곧 복잡도 지수**다. 삽입·버블이 `1.99`와 `2.00`으로
`O(n^2)`를 그대로 보여 주고, 블록 정렬은 `1.32`다. `n log n`이면 1.1쯤이어야 하니
조금 가파른 것이 `log^2` 항의 값이다.

| n | insertion(ms) | bubble(ms) | block(ms) |
| ---: | ---: | ---: | ---: |
| 1,000 | 0.635 | 1.624 | 0.129 |
| 2,000 | 2.549 | 6.948 | 0.306 |
| 4,000 | 10.274 | 28.399 | 0.724 |
| 8,000 | 41.083 | 121.502 | 1.748 |

![걸린 시간이 n에 따라 자라는 모양](docs/growth-time.svg)

시간도 같은 모양으로 자란다. 다만 비교 횟수만큼 매끈하지 않은데, 캐시와 스케줄러가
끼어들기 때문이다. **그래서 보고서에는 비교·이동 횟수를 함께 싣는다.**

`n = 8,000`에서 블록 정렬은 버블 정렬보다 **70배** 빠르다.
기울기가 다르므로 이 격차는 `n`이 커질수록 더 벌어진다.
