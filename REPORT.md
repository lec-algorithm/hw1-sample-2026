...
| --- | --- | ---: | ---: | ---: | ---: |
| 무작위 | insertionSort | 9.632 | 3,950,647 | 3,950,655 | 1 |
| 무작위 | bubbleSort | 27.865 | 7,997,922 | 11,828,007 | 1 |
| 무작위 | **blockSort** | **0.679** | **115,971** | 352,201 | 16 |
| 정렬됨 | insertionSort | 0.004 | 3,999 | 0 | 1 |
| 정렬됨 | bubbleSort | 0.004 | 3,999 | 0 | 1 |
| 정렬됨 | blockSort | 0.004 | 3,999 | 0 | 1 |
| 역순 | insertionSort | 19.602 | 8,001,999 | 8,005,998 | 1 |
| 역순 | bubbleSort | 29.101 | 7,998,000 | 23,994,000 | 1 |
| 역순 | **blockSort** | **0.502** | **133,851** | 311,166 | 24 |
| 중복많음 | insertionSort | 8.971 | 3,575,497 | 3,575,032 | 1 |
| 중복많음 | bubbleSort | 26.125 | 7,867,695 | 10,703,898 | 1 |
| 중복많음 | **blockSort** | **0.408** | **74,499** | 238,678 | 21 |

![입력 모양에 따른 비교 횟수](docs/input-shapes.svg)

세로축이 로그다. 그러지 않으면 `정렬됨` 막대가 바닥에 붙어 보이지 않는다.

- **정렬된 입력에서는 셋이 똑같다.** 비교 `n-1`번, 이동 0번. 조기 종료가 셋 다 걸린다.
- **역순은 `O(n^2)` 둘의 최악이다.** 삽입 정렬의 비교가 `8,001,999`로
  이론값 `n^2/2 = 8,000,000`과 맞는다.
- **블록 정렬만 입력 모양에 거의 흔들리지 않는다.** 중복이 많으면 오히려 빨라진다
  (조기 종료가 더 자주 걸린다).

그런데 역순에서 버블 정렬이 삽입 정렬보다 느린 이유는 비교가 아니다. 둘의 비교
횟수는 거의 같다.

![비교 횟수는 비슷한데 이동 횟수가 다르다](docs/compares-vs-moves.svg)

차이는 **이동**이다. 버블 정렬은 `23,994,000`회로 비교 횟수의 정확히 3배다.
매 비교가 교환으로 이어지고, 교환 하나가 `tmp`를 거쳐 이동 3회이기 때문이다. 시간이
삽입 정렬의 1.5배인 이유가 이것이다.
...
| ---: | ---: | ---: | ---: |
| 1,000 | 0.619 | 1.596 | 0.126 |
| 2,000 | 2.428 | 6.663 | 0.297 |
| 4,000 | 10.086 | 26.978 | 0.732 |
| 8,000 | 39.853 | 116.075 | 1.693 |
...
`n = 8,000`에서 블록 정렬은 버블 정렬보다 **69배** 빠르다.
